...
from app.utils import restore_user, validation_errors_to_error_messages
...
def login():
    ...
    if form.validate_on_submit():
        ...
        user = (
            User.query.filter(User.email == form.data["credential"]).first()
            or User.query.filter(User.username == form.data["credential"]).first()
        )
        login_user(user)
